...
`astrbot_plugin_groupJoinInspector` 以申请阶段审批为主，也包含轻量的入群后算术验证兜底；它不会维护黑名单、白名单、禁言观察列表，也不会因为算术验证失败而拉黑用户。

`astrbot_plugin_groupJoinFilter` 已弃用时，可以只保留本插件。若仍同时使用旧插件，需要避免两个插件同时处理同一批入群事件导致重复提示或重复踢人。
